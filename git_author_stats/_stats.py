    ...
    for url in urls:
        if is_github_organization(url):
            ...
            for repository_url in iter_github_organization_repository_urls(
                ...
                yield repository_url, clone(
                    repository_url, user=user, password=password, since=since
                )
        else:
            yield url, clone(url, user=user, password=password, since=since)
...
def iter_local_repo_stats(
    ...
    try:
        ...
        for line in filter(
            ...
            map(
                str.strip,
                check_output(command, text=True).strip().split("\n")
            )
        ):
