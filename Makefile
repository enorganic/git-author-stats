...
reinstall:
	...
	{ . venv/bin/activate || venv/Scripts/activate.bat ; } && \
	pip install --upgrade pip && \
	pip install flake8 mypy black tox pytest isort dependence daves-dev-tools -e . && \
	{ mypy --install-types --non-interactive || echo "" ; } && \
	make requirements && \
	echo "Installation complete"
...
upgrade:
	{ . venv/bin/activate || venv/Scripts/activate.bat ; } && \
	dependence freeze\
	 -nv '*' . pyproject.toml tox.ini daves-dev-tools dependence jupyter \
	 > .requirements.txt && \
...
requirements:
	...
	dependence freeze\
	 -e pip\
	 -e wheel\
	 . pyproject.toml tox.ini daves-dev-tools dependence jupyter \
	 > requirements.txt
